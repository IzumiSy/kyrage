...
const removeAutoGeneratedIndexesFromUniqueConstraints = (
  ...
  indexes.filter(
    (index) =>
      !constraints.unique.some(
        (uc) => uc.table === index.table && uc.name === index.name
      )
  );
*/
 ...
 * so there is no way to distinguish between automatically generated unique constraints and user-defined unique constraints.
 * This function searches for user-defined unique constraints and indexesin the configuration and adopts what is present in the configuration.
