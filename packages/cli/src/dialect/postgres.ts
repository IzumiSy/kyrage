...
export const postgresExtraIntrospectorDriver = (props: {
  ...
  return {
    ...
    introspect: async () => {
      ...
      return {
        tables: await introspectPSQLTables(db),
        indexes: await introspectPSQLIndexes(db),
        constraints: await introspectPSQLConstraints(db),
      };
    },
  };
};
...
const introspectPSQLIndexes = async (db: PlannableKysely) => {
  const { rows } = await sql`
    SELECT
...
const introspectPSQLConstraints = async (db: PlannableKysely) => {
  const { rows } = await sql`
    SELECT
      n.nspname AS schema,
      t.relname AS table,
