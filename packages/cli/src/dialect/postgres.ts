...
import { ConstraintAttributes, IndexAttributes, KyrageDialect } from "./types";
...
export const doPSQLintrospect = async (client: DBClient) => {
  ...
  const nomalizedIndexes = removeAutoGeneratedIndexesFromUniqueConstraints(
    indexes,
    constraints
  );
...
/*
 * Remove auto-generated indexes from unique constraints which has the same name of the index.
 * Ref: https://www.postgresql.org/docs/current/indexes-unique.html
 */
const removeAutoGeneratedIndexesFromUniqueConstraints = (
  indexes: IndexAttributes,
  constraints: ConstraintAttributes
) =>
  indexes.filter(
    (index) => !constraints.unique.some((uc) => uc.name === index.name)
  );
...
const introspectPSQLTables = async (db: PlannableKysely) => {
